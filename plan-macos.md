# macOS App 开发计划 (Phase 1)

## 项目概述

macOS 菜单栏/Dock 应用，提供截图翻译和本地单词本功能。

## 技术栈

| 组件 | 技术选型 |
|------|----------|
| UI 框架 | SwiftUI |
| 数据持久化 | SwiftData |
| OCR | Vision Framework (VNRecognizeTextRequest) |
| 翻译 | Apple Translation Framework |
| 截图 | ScreenCaptureKit |
| 快捷键 | NSEvent Global/Local Monitor |

## 架构设计

```
┌─────────────────────────────────────────────────────────────┐
│                        AppDelegate                          │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐ │
│  │ NSStatusItem│  │ Global      │  │ Window Management   │ │
│  │ (Menu Bar)  │  │ Hotkey      │  │                     │ │
│  └─────────────┘  └─────────────┘  └─────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌─────────────────────────────────────────────────────────────┐
│                        AppState                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │              ScreenshotTranslateViewModel            │   │
│  │  ┌───────────┐ ┌──────────┐ ┌───────────────────┐   │   │
│  │  │Screenshot │ │   OCR    │ │   Translation     │   │   │
│  │  │ Service   │ │ Service  │ │     Service       │   │   │
│  │  └───────────┘ └──────────┘ └───────────────────┘   │   │
│  └─────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │               WordBookViewModel                      │   │
│  │  ┌───────────────────────────────────────────────┐  │   │
│  │  │              WordBookManager                   │  │   │
│  │  │           (SwiftData CRUD)                     │  │   │
│  │  └───────────────────────────────────────────────┘  │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```

## 核心功能

### 1. 截图翻译流程

```
⌘+⇧+S 快捷键 (Cmd+Shift+S)
    │
    ▼
┌─────────────────┐
│ 全屏选区覆盖层   │ ← SelectionOverlayWindow (NSWindow, level: .floating)
│ 用户拖拽选择区域 │
└─────────────────┘
    │ ESC 取消 / 拖拽完成
    ▼
┌─────────────────┐
│ ScreenCaptureKit│ ← 捕获选中区域截图 (CGImage)
│ 截图捕获         │
└─────────────────┘
    │
    ▼
┌─────────────────┐
│ Vision OCR      │ ← VNRecognizeTextRequest 提取文字
│ 文字识别         │   支持中英文自动识别
└─────────────────┘
    │
    ▼
┌─────────────────┐
│ Apple Translate │ ← TranslationSession (via SwiftUI bridge)
│ 翻译             │   自动检测源语言 → 中文
└─────────────────┘
    │
    ▼
┌─────────────────┐
│ 结果展示窗口     │ ← TranslationResultWindow (NSPanel)
│ 原文 + 译文      │   可收藏到单词本
└─────────────────┘
```

### 2. 单词本功能

- SwiftData 本地持久化
- 支持搜索、删除
- 记录来源 (screenshot/webpage/video)
- 后续可通过 Firebase 云同步

## 文件结构

```
TranslatorApp/
├── TranslatorApp.xcodeproj
├── TranslatorApp/
│   ├── App/
│   │   ├── TranslatorAppApp.swift      # 入口 + AppDelegate
│   │   └── AppState.swift              # 全局状态管理
│   │
│   ├── Core/
│   │   ├── Errors/
│   │   │   └── TranslatorError.swift   # 统一错误类型
│   │   ├── Models/
│   │   │   └── Word.swift              # SwiftData 模型
│   │   └── Services/
│   │       ├── Protocols.swift         # 服务协议定义
│   │       ├── ScreenshotService.swift # 截图服务
│   │       ├── OCRService.swift        # OCR 服务
│   │       ├── TranslationService.swift# 翻译服务
│   │       └── WordBookManager.swift   # 单词本管理
│   │
│   ├── Features/
│   │   ├── ScreenshotTranslate/
│   │   │   ├── ScreenshotTranslateViewModel.swift
│   │   │   ├── SelectionOverlayWindow.swift
│   │   │   └── TranslationResultWindow.swift
│   │   └── WordBook/
│   │       ├── WordBookViewModel.swift
│   │       └── WordBookView.swift
│   │
│   └── Info.plist
```

## 数据模型

```swift
@Model
final class Word {
    @Attribute(.unique) var id: UUID
    var text: String           // 原文
    var translation: String    // 译文
    var source: String         // 来源: screenshot, webpage, video
    var sourceURL: String?     // 来源 URL
    var tags: [String]         // 标签
    var createdAt: Date        // 创建时间
    var syncedAt: Date?        // 同步时间
}
```

## 权限要求

| 权限 | 用途 | Info.plist Key |
|------|------|----------------|
| 屏幕录制 | ScreenCaptureKit 截图 | NSScreenCaptureUsageDescription |
| 辅助功能 | 全局快捷键监听 | 系统设置手动授权 |

## 已完成功能 ✅

- [x] 项目结构搭建
- [x] 菜单栏/Dock 应用框架
- [x] 全局快捷键 ⌘+⇧+S (Cmd+Shift+S)
- [x] 截图选区功能 (SelectionOverlayWindow)
- [x] ScreenCaptureKit 截图
- [x] Vision OCR 文字识别
- [x] Apple Translation 翻译
- [x] 翻译结果展示窗口
- [x] 收藏到单词本
- [x] 单词本界面 (WordBookView)
- [x] SwiftData 持久化

## 已修复问题 🔧

1. **系统死锁**: 选区窗口 level 从 `.screenSaver` 改为 `.floating`
2. **键盘监听泄漏**: 添加 `cleanupKeyboardMonitor()` 清理
3. **选择超时**: 添加 60 秒超时机制
4. **窗口消失**: 添加 `resultWindow` 强引用防止释放
5. **Translation API**: 使用 SwiftUI `.translationTask` bridge 模式

## 待优化项目 📋

- [ ] 菜单栏图标在 macOS 26 显示问题
- [ ] 多显示器支持
- [ ] 历史记录查看
- [ ] 快捷键自定义
- [ ] 翻译目标语言设置

## Phase 2: Native Messaging Host ✅ 已完成

### 实现细节

Chrome 扩展通过 Native Messaging 与 macOS App 通信：

```
Chrome Extension (content.ts)
       ↓ chrome.runtime.sendMessage
Background Service Worker (background.ts)
       ↓ NATIVE_MESSAGE proxy
       ↓ chrome.runtime.sendNativeMessage
macOS App (NativeMessagingHost)
       ↓
TranslationService / HostWordBookService
       ↓
SwiftData (~/Library/Application Support/com.translator.app/default.store)
```

### 关键实现

1. **NativeMessagingHost**: `TranslatorApp/NativeMessagingHost/` 目录
   - `NativeMessagingHost.swift` - 主程序，处理 stdin/stdout 消息
   - `HostWordBookService.swift` - 单词本服务，写入共享 SwiftData

2. **SwiftData 路径统一**：
   - NativeMessagingHost 和 TranslatorApp 必须使用相同路径
   - 路径：`~/Library/Application Support/com.translator.app/default.store`
   - AppState.swift 使用显式 `ModelConfiguration(url: storeURL)`

3. **Model 类名统一**：
   - 两边都必须使用 `Word` 类（不是 `HostWord`）
   - SwiftData 通过类名匹配 schema

4. **Native Messaging Manifest**:
   - 位置：`~/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.translator.app.json`
   - 指向：`NativeMessagingHost/.build/debug/NativeMessagingHost`

### 已完成功能

- [x] NativeMessagingHost 可执行文件
- [x] 处理 translate 请求
- [x] 处理 save_word 请求
- [x] 处理 ping 健康检查
- [x] 与 TranslatorApp 共享 SwiftData 数据库
- [x] Chrome 扩展单词收藏同步到 macOS 单词本

## 后续阶段

### Phase 3: 视频字幕翻译
- YouTube 字幕提取
- Bilibili 字幕翻译

### Phase 4: Flutter 云同步
- Firebase Firestore 集成
- 多设备单词本同步
